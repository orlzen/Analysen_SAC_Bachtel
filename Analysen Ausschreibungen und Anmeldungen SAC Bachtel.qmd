...
### Anmeldungen von Einzelpersonen

```{r}
# Anzahl eindeutige Anmeldungen pro Jahr und Mitglied
anm_unique_jahr_member <- anm_total |> 
  group_by(TN_Kuerzel, jahr) |> 
  summarise(freq = n()) |> 
  arrange(desc(freq))

# Anzahl eindeutige Anmeldungen insgesamt pro Mitglied
anm_unique_member <- anm_total |> 
  group_by(TN_Kuerzel) |> 
  summarise(freq = n()) |> 
  arrange(desc(freq))
```

Neben der Anzahl Anmeldungen kann auch analysiert werden, *wie viele einzelne* Anmeldungen eingegangen sind, d.h. wie viele *Personen* sich auf die Touren anmelden. Dies kann anhand der Variable `TN_Kuerzel` gemacht werden. In allen untersuchten Anmeldungen der Jahre 2016 bis 2022 verzeichnet die Person mit den meisten Anmeldungen `r max(anm_unique$freq)` Anmeldungen auf Anlässe der Sektion.[^1] In @tbl-max-anm-unique-jahr ist die höchste Anzahl der Anmeldungen von einer einzelnen Person pro Jahr ersichtlich. 

[^1]: Aus Datenschutzgründen werden die Kürzel der betreffenden Personen nicht wiedergegeben.

```{r}
#| label: tbl-max-anm-unique-jahr
#| tbl-cap: "Maximum der Anmeldungen von Einzelpersonen pro Jahr"


# Anzahl eindeutige Anmeldungen pro Jahr
knitr::kable(anm_unique_jahr_freq <- anm_total |> 
  group_by(TN_Kuerzel, jahr) |> 
  summarise(freq = n()) |> 
  group_by(jahr) |> 
  summarise(freq_max = max(freq)),
  col.names = c("Jahr", "Max. Anmeldungen"),
  align = "lc")
```

```{r}
#| label: tbl-anm-unique-jahr
#| tbl-cap: "Anzahl Anmeldungen von Einzelpersonen pro Jahr"
...
knitr::kable(anm_total |> 
  ...
  summarise(freq = n()) |> 
  group_by(jahr) |> 
  summarise(freq_unique = unique(freq)),
  col.names = c("Jahr", "Anzahl eindeutige Anmeldungen"),
  align = "lc")


anm_uniqe_jahr <- anm_total |> 
  group_by(jahr, TN_Kuerzel) |> 
  summarise(freq_unique = nrow(unique(TN_Kuerzel)))
```


### Spezialfall Sektionsversammlung
Die Sektionsversammlung ist die entscheidende Zusammenkunft der Sektionsmitglieder, wo das Budget und allgemeine Informationen präsentiert werden und alle zwei Jahre der Vorstand wieder- oder neugewählt wird. Umso relevanter wäre eine grosse Anwesenheit der Sektionsmitglieder an diesem Anlass.
...
In @fig-ausschreibungen-tourenstatus ist der Tourenstatus der ausgeschriebenen Touren pro Jahr ersichtlich, wobei wiederum für das Corona-Jahr 2020 der hohe Anteil "abgesagt" ersichtlich ist. Der eher höhere Anteil der abgesagten Touren im Jahr 2022 dürfte primäer auf Sommertouren und auf die schlechten Schneeverhältnisse auf Gletschern zurückzuführen sein (siehe auch @fig-ausschreibungen-tourenstatus-tourtyp, wo der Tourtyp der abgesagten Ausschreibungen ersichtlich ist. Im Jahr 2022 lag die Anzahl abgesagter Hochtouren um einiges höher als in den Vorjahren (mit Ausnahme des Corona-Jahres 2020)). Auf die Darstellung des Jahres 2023 wurde hier verzichtet, da sich dies noch ändern kann.
...
```{r}
#| label: fig-ausschreibungen-tourenstatus-tourtyp
#| fig-cap: "Tourtyp der abgesagten Ausschreibungen nach Jahr, nur Anlässe der Sektion (ohne Senioren oder JO)"


auss_total |> 
  filter(jahr != 2023) |> 
  filter(Gruppe %in% c("Sektion")) |> 
  filter(!`Tourtyp lang` %in% "JO Skitour") |> 
  filter(Status %in% "abgesagt") |> 
  ggplot(aes(x = jahr,
             fill = factor(`Tourtyp lang`))) +
  geom_bar(stat = "count") +
  scale_fill_brewer(palette = "Set1") +
  labs(
    x = "Jahr",
