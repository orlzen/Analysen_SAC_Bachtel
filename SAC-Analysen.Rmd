...
```{r Einlesen_Daten_Anmeldung _und_Ausschreibungen, include = FALSE}
...
anm_2021 <- read_excel("Rohdaten/Anmeldungen 2021.xlsx")
...
auss_2022 <- read_excel("Rohdaten/Ausschreibungen 2022.xlsx")
```

## Anmeldungen

```{r Anmeldungen_bearbeiten, include = FALSE}
temp2016 <- c(
...
```{r create_anm_total, include = FALSE}
...
```{r anmeldungen_bereinigen, include = FALSE}
...
```{r Ausschreibungen_bearbeiten, include = FALSE}
...
# Status korrigieren (alt: "/ausgebucht", neu: "ausgebucht" [ohne slash])
auss_total$Status <- str_replace_all(auss_total$Status, '/', '')
```

```{r Datensätze Anmeldungen & Ausschreibungen zusammenfügen, include = FALSE}
...
```{r anmeldungen_2021, fig.cap="Verlauf der Anmeldungen im Jahr 2021 (alle Anlässe der Sektion)"}
anm_total %>%
